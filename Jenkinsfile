pipeline {
  ...
  environment {
    ...
    KUBE_NS       = 'registry'
    REGISTRY_SVC  = 'registry'      // имя Service c type=NodePort
  ...
  stages {
    ...
    stage('Kubectl version') {
      steps {
        withCredentials([file(credentialsId: 'kubeconfig_k8s', variable: 'KUBECONFIG')]) {
          sh '''
  ...
  post {
    success {
      withCredentials([
        file(credentialsId: 'kubeconfig_k8s',    variable: 'KUBECONFIG'),
        string(credentialsId: 'telegram-bot-token', variable: 'TG')
      ]) {
        sh '''
          set -e
          NODEIP=$(kubectl get nodes -o jsonpath="{.items[0].status.addresses[?(@.type=='InternalIP')].address}")
          NODEPORT=$(kubectl -n ${KUBE_NS} get svc ${REGISTRY_SVC} -o jsonpath="{.spec.ports[0].nodePort}")
